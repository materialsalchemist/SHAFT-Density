...
class MEGNetBackbone(torch.nn.Module):
    ...
    def forward(self, s):
        ...
        if len(x_atom.shape) == 1:
            x_atom = torch.unsqueeze(x_atom,dim=0)    
...
class GridMEGNetBackbone(torch.nn.Module):
    def __init__(self, dim=4, hidden_dim=128, n_hidden=2, output_dim=32,use_pretrain=True,pretrain_dim=128):
        super().__init__()
        bond_expansion = BondExpansion(rbf_type="Gaussian", initial=0.0, final=5.0, num_centers=100, width=0.5)
        ...
        self.emb_min_density = nn.Linear(3, 32)
        ...
        self.emb_sg = nn.Embedding(230, output_dim)
        if use_pretrain:
            self.output_layer = nn.Linear(hidden_dim*4+32, output_dim)
        else:
            self.output_layer = nn.Linear(hidden_dim*3+32, output_dim)

        self.use_pretrain = use_pretrain


    def forward(self, s):
        graph, sg, lattice, fpretrain, min_density = s
        g, edge_feat, node_feat, state_attr = graph
        
        x_atom = self.backbone(g, edge_feat, node_feat, state_attr)
        x_lattice = self.emb_lattice(lattice)

        x_min_density = self.emb_min_density(min_density)
        ...
        if self.use_pretrain:
            x_pretrain = self.emb_pretrain(fpretrain)
            out = torch.cat((x_atom, x_lattice,x_pretrain,x_sg, x_min_density), dim=1)
        else:                
            out = torch.cat((x_atom, x_lattice, x_sg, x_min_density), dim=1)
