...
class HierarchicalActionSampler(ActionsSampler):
    ...
    def __init__(self, estimators: List[GCNBackbone], req_config: List,
                 ...
                 atom_mask_func='get_valid_mask_atom_type') -> None:
    ...
    def lattice_sample_to_action(self, lattice):
        lattice[:,0:3] = (self.max_length-self.min_length)*lattice[:,0:3] + self.min_length
        lattice[:,3:] = (self.max_angle-self.min_angle)*lattice[:,3:] + self.min_angle
        return lattice
    
    def lattice_action_to_sample(self, lattice_action):
        lattice_action[:,0:3] = (lattice_action[:,0:3] - self.min_length)/(self.max_length-self.min_length)
        lattice_action[:,3:] = (lattice_action[:,3:] - self.min_angle)/(self.max_angle-self.min_angle)
        return lattice_action

    def adjust_lattice_based_on_action(self, lattices, sg):
        spacegroup_action = sg.clone().cpu().detach().numpy()
        spacegroup_action = np.squeeze(spacegroup_action) if len(spacegroup_action.shape) != 1 else spacegroup_action
    ...
    def sample(self,states,states_reps,
        ...
        mask = torch.tensor(np.array([state.get_valid_mask_sg_from_requirement() for state in states])).to(device=self.device)
        logits_esg[1] = torch.where(mask, logits_esg[1], -float("inf"))
        ...
        with torch.no_grad():
            if step > self.min_stop:
                ...
                exit_action = torch.zeros(len(states),dtype=torch.float).to(device=self.device)
        ...
        if len(self.req_config['req_ele']) != 0: 
            mask = torch.tensor(np.array([state.get_valid_mask_atom_type(func_name=self.atom_mask_func,max_traj_len=maxblock) for state in states])).to(device=self.device)
