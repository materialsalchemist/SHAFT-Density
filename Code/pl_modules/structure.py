...
class BaseCrystalStructure:
    ...
    def get_valid_mask_sg_from_requirement(self, size=230):
        result = [False] * size
    
        # Set True for the given indices
        for idx in self.req_config['sg_list']:
            if 0 <= idx < size:  # Ensure the index is within the valid range
                result[idx] = True
        return result

    
    def get_valid_mask_atom_type(self, func_name, max_traj_len):
        if func_name == 'get_valid_mask_atom_type_one_group':
            return self.get_valid_mask_atom_type_one_group(max_traj_len)
        elif func_name == 'get_valid_mask_atom_type_simple':
            return self.get_valid_mask_atom_type_simple(max_traj_len)
        elif func_name == 'get_valid_mask_atom_type_two_group':
            return self.get_valid_mask_atom_type_two_group(max_traj_len)
        elif func_name == 'get_valid_mask_atom_type_one_group_with_Al':
            return self.get_valid_mask_atom_type_one_group_with_Al(max_traj_len)
        else:
            raise NotImplementedError

    def get_valid_mask_atom_type_one_group(self, max_traj_len):
    ...
    def get_valid_mask_atom_type_one_group_with_Al(self, max_traj_len):
        # check number of element used
        used_ele = set([e.symbol for e in self.atoms])
        used_ele = [self.req_config['ele_choice'].index(e) for e in used_ele]
        num_ele_used = len(used_ele)
        ...
        if num_ele_used == self.req_config['max_ele'] - 1:
            ...
            if not res:
                ...
                if self.req_config['ele_choice'].index('Al') not in used_ele:
                    mask = [e == self.req_config['ele_choice'].index('Al') for e in range(self.req_config['len_ele_list'])]
                else:   
                    mask = [True] * self.req_config['len_ele_list']
            return mask
        if num_ele_used < self.req_config['max_ele'] - 1:
    ...
    def get_valid_mask_atom_type_simple(self, max_traj_len):
        # check number of element used
        if len(self.atoms) == 0:
            mask = [e in self.req_config['req_ele'] for e in range(self.req_config['len_ele_list'])]
        mask = [True] * self.req_config['len_ele_list']
    ...
    def get_valid_mask_atom_type_two_group(self, max_traj_len):
        # check number of element used
        used_ele = set([e.symbol for e in self.atoms])
        used_ele = [self.req_config['ele_choice'].index(e) for e in used_ele]
        num_ele_used = len(used_ele)
        ...
        if num_ele_used == self.req_config['max_ele'] - 1:
            ...
            if not res:
                ...
                # pick atoms that are not in the requirement set
                mask = [e not in self.req_config['req_ele'] for e in range(self.req_config['len_ele_list'])]
            return mask
        if num_ele_used < self.req_config['max_ele'] - 1:
            if len(self.atoms) == max_traj_len - 1:
                res = any(ele in self.req_config['req_ele'] for ele in used_ele)
    ...
    def structure(self):
        ...
        try:
            self._structure.merge_sites(tol=1.0, mode='delete')
        except:
            print('Can not merge')
